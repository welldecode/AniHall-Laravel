...
import { RiArrowDownCircleLine} from "@remixicon/vue";
    ...
    <div class="flex items-center justify-center space-x-2">
      ...
      <template v-if="table && sort">
        <div class="flex flex-col items-center justify-center text-xs">
          <RiArrowDownCircleLine class="transition-all duration-300" :class="table.config.order.key === name && table.config.order.dir === 'asc' && asc ? 'text-black dark:text-white' : ' dark:text-gray-400 text-gray-400'" />
          <RiArrowDownCircleLine class="transition-all duration-300" :class="table.config.order.key === name && table.config.order.dir === 'desc' && desc ? 'text-black dark:text-white' : 'dark:text-gray-400 text-gray-400'" />
        </div>
      </template>
    </div>
